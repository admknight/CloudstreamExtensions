...
class Cinevood : MainAPI() {
        ...
        return if (isSingle) {
            ...
            val linkCollection = doc.select("a[href*='oxxfile']").mapNotNull { anchor ->
                ...
                items.mapNotNull { element ->
                    ...
                    val epTitle = Regex("""S\d+E\d+""").find(fullName ?: "")?.value ?: fullName
                    if (epTitle != null && link != null) Pair(epTitle, link) else null
            ...
            val episodes = linkCollection.flatten().map { (epTitle, link) ->
                newEpisode(listOf(link).toJson()) {
                    this.name = epTitle
                }
            }
    ...
    override suspend fun loadLinks(
        ...
        val links = tryParseJson<List<String>>(data)?.filterNot { it.isBlank() } ?: data.trim()
            .removePrefix("[")
            .removeSuffix("]")
            .split(',')
            .mapNotNull { raw -> raw.trim().removeSurrounding("\"").removeSurrounding("'").takeIf { it.isNotBlank() } }
