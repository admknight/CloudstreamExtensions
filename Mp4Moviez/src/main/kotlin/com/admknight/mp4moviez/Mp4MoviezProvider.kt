...
class Mp4MoviezProvider : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val doc = app.get("$mainUrl/search/$query.html", allowRedirects = true, timeout = 30).document
    ...
    private fun getVideoQuality(url: String?) =
        url?.substringAfter("q=")?.substringBefore("&")?.toIntOrNull() ?: Qualities.Unknown.value
}
