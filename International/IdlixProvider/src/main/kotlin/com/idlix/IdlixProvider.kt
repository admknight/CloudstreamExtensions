...
class IdlixProvider : MainAPI() {
    ...
    override suspend fun getMainPage(
        ...
        val home = res.data.map { item ->
            ...
            val scoreValue = Score.from10(item.voteAverage)
    ...
    override suspend fun load(url: String): LoadResponse {
        val data = app.get(url, timeout = 10000L).parsedSafe<DetailResponse>() ?: throw ErrorLoadingException("Invalid JSON")
        ...
        val actors = data.cast?.map { Actor(it.name ?: "", it.profilePath?.let { p -> "https://image.tmdb.org/t/p/w185$p" }) } ?: emptyList()
        val trailer = data.trailerUrl
        val scoreValue = Score.from10(data.voteAverage?.toString())

        val relatedUrl = if (data.seasons != null) "$mainUrl/api/series/${data.slug}/related" else "$mainUrl/api/movies/${data.slug}/related"
        val weburl = if (data.seasons != null) "$mainUrl/series/${data.slug}" else "$mainUrl/movie/${data.slug}"

        val recommendations = runCatching {
            app.get(relatedUrl, referer = mainUrl).parsedSafe<ApiResponse>()?.data?.mapNotNull { item ->
                val rTitle = item.title ?: return@mapNotNull null
                val rPoster = item.posterPath?.let { "https://image.tmdb.org/t/p/w342$it" }
                val rLink = if (item.contentType == "movie") "$mainUrl/api/movies/${item.slug}" else "$mainUrl/api/series/${item.slug}"
                
                newMovieSearchResponse(rTitle, rLink, if (item.contentType == "movie") TvType.Movie else TvType.TvSeries) {
        ...
        if (data.seasons != null) {
            ...
            data.firstSeason?.episodes?.forEach { ep ->
                ...
                episodes.add(newEpisode(LoadData(id = epId, type = "episode").toJson()) {
                    this.name = ep.name
            ...
            data.seasons.forEach { season ->
                ...
                val seasonData = runCatching { app.get("$mainUrl/api/series/${data.slug}/season/$seasonNum", referer = mainUrl).parsedSafe<SeasonWrapper>()?.season }.getOrNull()
            ...
            return newTvSeriesLoadResponse(title, weburl, TvType.TvSeries, episodes) {
                this.posterUrl = poster
            ...
            return newMovieLoadResponse(title, weburl, TvType.Movie, LoadData(id = data.id ?: "", type = "movie").toJson()) {
                this.posterUrl = poster
                this.backgroundPosterUrl = backdrop
                this.logoUrl = logourl
                this.year = year
    ...
    override suspend fun loadLinks(
        ...
        val parsed = try { parseJson<LoadData>(data) } catch (_: Exception) { null } ?: return false
        val contentId = parsed.id
        val contentType = parsed.type
        val headers = mapOf("Referer" to "$mainUrl/", "Origin" to mainUrl, "Accept" to "*/*", "Content-Type" to "application/json")

        val playResponse = app.get("$mainUrl/api/watch/play-info/$contentType/$contentId", headers = headers)
        ...
        val claimApi = app.post("$mainUrl/api/watch/session/claim", headers = headers, cookies = cookies, requestBody = """{"gateToken": "${playInfo.gateToken}"}""".toRequestBody("application/json".toMediaType())).parsedSafe<RedeemRes>() ?: return false
        val iframeResponse = app.post(claimApi.redeemUrl, headers = headers, cookies = cookies, requestBody = """{"claim": "${claimApi.claim}"}""".toRequestBody("application/json".toMediaType())).parsedSafe<Iframe>() ?: return false
        ...
        iframeResponse.subtitles.forEach { subtitle ->
            subtitleCallback(newSubtitleFile(subtitle.label, subtitle.path))
        }
...
data class Res(val gateToken: String, val serverNow: Long, val unlockAt: Long)
data class RedeemRes(val kind: String, val claim: String, val redeemUrl: String, val videoId: String, val title: String, val durationSec: Long, val viewerTier: String, val maxHeight: Long)
data class Iframe(val code: String? = null, val url: String? = null, val expiresAt: Long? = null, val subtitles: List<Subtitle> = emptyList(), val videoId: String? = null)
data class Subtitle(val lang: String, val label: String, val path: String)
