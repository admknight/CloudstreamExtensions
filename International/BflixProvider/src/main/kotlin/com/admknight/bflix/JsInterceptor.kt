...
class JsInterceptor(private val serverid: String) : Interceptor {
    ...
    private fun Handler.postFunction(function: () -> Unit) {
    ...
    private fun resolveWithWebView(request: Request): Request? {
        ...
        handler.postFunction {
            ...
            with(webview.settings) {
                ...
                userAgentString = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36"
                ...
                webview.webViewClient = object : WebViewClient() {
                    override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
                        ...
                        if (request?.url.toString().contains(Regex("list.m3u8|/simple/"))) {
                            newRequest = requestCreator("GET", request?.url.toString(), headers = mapOf("referer" to "https://vidstream.pro/"))
                            latch.countDown()
                            return null
                        }
