    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        if (tmdbIdResolved.isBlank()) {
            try {
                ...
                val json = JSONObject(app.get("$TMDBAPI/search/$type?api_key=$TMDBAPIKEY&query=$title").text)
                tmdbIdResolved = json.optJSONArray("results")?.optJSONObject(0)?.optInt("id")?.toString().orEmpty()
            } catch (_: Exception) {}
        }
            ...
            seasons.forEach { seasonNum ->
                try {
                    ...
                    for (i in 0 until arr.length()) {
                        ...
                        val epRating = Score.from10(ep.optDouble("vote_average", 0.0))
                    ...
                    try {
                        ...
                        if (episodeBlocks.isNotEmpty()) {
                            episodeBlocks.forEach { h5 ->
                                val epNum = Regex("""Episodes:\s*(\d+)""").find(h5.text())?.groupValues?.get(1)?.toIntOrNull() ?: return@forEach
        ...
        val episodes = episodesMap.map { (key, links) ->
            ...
            newEpisode(links.distinct().toJson()) {
                this.name = meta?.title ?: "Episode $epNum"
                this.season = seasonNum
                this.episode = epNum
                this.description = meta?.overview ?: plot
        ...
        if (tmdbIdResolved.isNotBlank()) {
            try {
                val type = if (isMovie) "movie" else "tv"
                val json = JSONObject(app.get("$TMDBAPI/$type/$tmdbIdResolved?api_key=$TMDBAPIKEY&append_to_response=credits").text)
                ...
                json.optJSONObject("credits")?.optJSONArray("cast")?.let { castArr ->
                        for (i in 0 until castArr.length()) {
                            ...
                            val actorName = c.optString("name").takeIf { it.isNotBlank() } ?: c.optString("original_name")
                            ...
                            actorDataList += ActorData(Actor(actorName, profile), roleString = character)
                        }
                    }
