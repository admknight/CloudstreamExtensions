    ...
    override suspend fun loadLinks(
        ...
        callback: (ExtractorLink) -> Unit
         ...
         /*  app.get(data).document.select("ul.ListOptions li").forEach {
               ...
               doc1.select("div.Video iframe").map {
                ...
                ).okhttpResponse.headers.values("location").map { link ->
                    val url1 = link.replace("#bu", "")
                    loadExtractor(url1, data, subtitleCallback, callback)
                }
            }
        } */
        app.get(data).document.select("li div.Button.sgty").map {
